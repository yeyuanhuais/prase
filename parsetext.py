...
def process_message_0x01(msg,time):
    for key in id_mapping:
        plcs=find_keys_by_value(plc_mapping,key)
        if len(plcs)>0:
            for plc in plcs:
...
def process_start_plc_0x30(line):
    ...
    if len(data):
        ...
        val = data["data"].split("|@")
        val=val[len(val)-2]
        plc_lasts=find_keys_by_value(value_last_mapping,int(val))
        if len(plc_lasts)>0:
            for plc_last in plc_lasts:
                plc_start=int(plc_last)-PACKAGE_LENGTH+1
                indextwo = addr_mapping[plc_start]
                indexone = map_addr_to_index(plc_start)
                if str(indextwo) in parsed_data[0]:
...
def process_read_plc_0x30(line):
    ...
    if len(data):
        val = data["data"].split("|@")
        val=val[len(val)-2]
        value_after_addrs=find_keys_by_value(value_last_mapping,int(val))
        if len(value_after_addrs)>0:
            for value_after_addr in value_after_addrs:
...
def process_receive_0x0d(line):
    ...
    if len(data):
        ...
        plcs=find_keys_by_value(plc_mapping,int(last_key)-PACKAGE_LENGTH+1)
        if len(plcs)>0:
            for plc in plcs:
                plc_key=plc+PACKAGE_LENGTH-1
...
def process_message_split(msg, line):
    # 创建正则表达式，排除完全为0的项
    pattern = re.compile(r'(?<!\d)(?!�)(' + '|'.join(map(str, id_mapping.keys())) + r')\|@(?!(0\|;){6,})')
    # 查找所有匹配项
    ...
    if len(matches)>0:
        ...
        for item in matches:
            key, value = item  # 解包元组
            indextwo = id_mapping[int(key)]
            indexone = map_id_to_index(int(key))
            if str(indextwo) not in parsed_data[0]:
                data_templates["node_receive_0x01"][indexone][indextwo+1]=time
                continue
            parsed_data_index = parsed_data[0].index( str(indextwo) )
...
def process_id_fields(msg, line):
    ...
    if len(matches)>0:
        ...
        for item in matches:
            ...
            indexone = map_id_to_index(int(key))
            if str(indextwo) not in parsed_data[0]:
                data_templates["www_receive_0x01"][indexone][indextwo+1]=time
                continue
            parsed_data_index = parsed_data[0].index( str(indextwo) )
